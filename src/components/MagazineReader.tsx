...
export default function MagazineReader({ issue, stories, preview = false }: { issue: Edition; stories: Story[]; preview?: boolean }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setSpread(query.matches ? preview : false);
    };
  ...
  return <div className="edition-shell">
    ...
    <div className="edition-heading"><div><p className="eyebrow">{preview ? 'Private issue preview' : 'The weekly technology edition'}</p><h1>{preview ? 'Review the complete draft.' : 'This week, in perspective.'}</h1><p>{preview ? 'Every story below remains private until you publish this issue.' : 'Read across technology. Save what matters. Find it again.'}</p></div><div className="edition-actions"><Link href={preview ? '/admin' : '/search'}>{preview ? 'Back to dashboard' : 'Explore the archive'}</Link>{canAnimate && <button onClick={() => setSpread(!spread)} aria-pressed={spread}>{spread ? 'Section view' : 'Magazine view'}</button>}</div></div>
    ...
    {spread && canAnimate ? <FlipBook layout={issue.layout} preview={preview}>{generateBroadsheetPages(issue, stories.filter(s => s.domain !== 'Drama'), stories.filter(s => s.domain === 'Drama'))}</FlipBook> : <div className="edition-layout">
