...
export function parseAndValidateDraft(output: string, domainHint?: string | null): ValidatedDraft {
  ...
  if (wordCount < 140 || wordCount > 650) {
    throw new Error(`The draft contains ${wordCount} words; the accepted range is 140-650.`);
  }
  const paragraphCount = crux.split(/\n\s*\n/).filter(Boolean).length;
  if (paragraphCount < 3) throw new Error(`The draft contains ${paragraphCount} paragraph; at least 3 are required.`);
