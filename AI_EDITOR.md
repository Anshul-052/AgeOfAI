...
The inbox refreshes queued work automatically. Each card shows the assigned provider and model, routing reason, job state, word count, token count, fallback use, and errors. The local writer targets a source-adaptive 120-180 word article that explains the event, its context, and its practical impact. A thin result receives up to three evidence-grounded completeness rewrites, and the longest valid version is kept. This is generation guidance rather than an acceptance threshold: a usable short result is preserved if the model cannot safely expand it. Empty output and stories above the 650-word display ceiling are rejected. A drafted story can be selected and queued again for rewriting. `npm run editor:local-once` processes one queued story and exits, which is useful for testing. The worker needs the same `DATABASE_URL` as the deployed application because Vercel cannot connect directly to a model running on a private laptop.
