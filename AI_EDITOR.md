...
The inbox refreshes queued work automatically. Each card shows the assigned provider and model, routing reason, job state, word count, token count, fallback use, and errors. The local writer aims for a source-adaptive 120-180 word article that explains the event, its context, and its practical impact. A first draft below 80 words receives one best-effort expansion pass, but there is no lower word-count acceptance gate: the second usable draft is preserved when the evidence genuinely supports only a brief. Empty output and stories above the 650-word display ceiling are rejected. `npm run editor:local-once` processes one queued story and exits, which is useful for testing. The worker needs the same `DATABASE_URL` as the deployed application because Vercel cannot connect directly to a model running on a private laptop.
